...
function Add(){
	...
	url = getContextPath()+"/District/addOne";
}
...
function ModifyBySelect(){
	...
	url = getContextPath()+"/District/updateOne";  //设置修改的地址
}
...
function DeleteByFruitName(){
	...
	$.messager.confirm("系统提示", "你确定要删除<font color=red> " + SelectRows.length + " </font>条数据吗?", function(xo){
		if( xo ){
			...
			$.post(getContextPath()+"/District/deleteBySomething",{xhs:SelectIndexToString},function(result){
				if( result.result>=1){
